...
class Orders(db.Model):
	...
	def __init__(self,id_order, ed_index, cust_id, period_id,quantity,  order_date, total_cost):
...
def start_point():
	...
	if gsearch:
		...
		date_perion_week = []
		for i in range(7):
			date_perion_week.append(now.strftime("%d-%m-%Y"))
			now = now - time_delta


		all_orders = db.session.query(Orders.ed_index, db.func.count(Orders.ed_index)).join(Editions).filter(Orders.order_date.in_(date_perion_week)).group_by(Orders.ed_index).order_by(db.desc(db.func.count(Orders.ed_index))).limit(10).all()
		index_top = []
		for row in all_orders:
			index_top.append(row[0])
		top_edition = db.session.query(Editions).filter((Editions.edit_id).in_(index_top))
		

		#popular_editions = db.session.query(Editions).filter_by(category = 'Популярні видання').all()
		return render_template('index.html', editions = top_edition, category = 'Популярні видання')
...
def create_pie(x1, x2):
    ...
    fig = go.Figure(data=[go.Pie(labels=labels, values=values, textinfo='label+percent', title = "Популярнісь категорій")])
...
def plot():
	...
	values_category = []
	count_category = []
	data_plot = db.session.query(Editions.category, db.func.count(Orders.ed_index)).outerjoin(Orders, Editions.edit_id == Orders.ed_index).group_by(Editions.category).all()
	for i in data_plot:
		values_category.append(i[0])
		count_category.append(i[1])

	pie_graph = create_pie(values_category, count_category)
...
def user_info():
	if request.method == "POST":
		...
		if len(period)==0:
			...
			return render_template('user_infor.html', edition=edition, period = period, quantity = quantity, total_cost = float(total_cost))
...
def confirm_address():
	if request.method == "POST":
		...
		edit_price = db.session.query(Editions.price, Editions.edit_id).filter(Editions.edit_id == int(index_edition)).first()
		month_count = period.split('/')
		period_month = len(month_count)

		total_cost = (edit_price.price)*period_month*quantity
